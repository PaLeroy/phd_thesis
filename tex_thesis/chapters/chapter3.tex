...
\todo{From the introduction, the mode of executions and training, based on the special case of oliehook, MMDP, MPOMDP, observability, central model, factored model, show how we can have the centralised paradigm}.
...
\todo{Convergence ? Solutions? Nash etc? + intérêt d'avoir une seule reward, cf oliehoek}.

\todo{Oliehoek shown how to solve dec-pomdp with RL as one of the furhter topic}
...
This chapter aims to define the CTDE methods compared to later defined ones.
While we have already provided other existing methods, we will discuss different topics of MARL, mainly related to the cooperative setting.
...
Mean-field game is a different way of modelling the framework \citep{lauriere2022learning}.
...
An agent decides on a message and an action based on its observation of the environment and messages sent by other agents.
