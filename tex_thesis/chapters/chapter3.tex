...
It is theindependent method corresponding to IQL for policy-based methods.
As defined in Section \ref{sec:ch3_decpomdp}, the policies learned by each actor is $\pi^{a}(u_t^{a}|\tau_t^{a}, o^a_t;\theta)$.
...
The parameters are denoted $\theta$ or $\phi$ and not $\theta_a$ or $\phi_a$ to improve readability, but also because it is possible to share these parameters to train a single network.

In IAC, each agent learns an actor and a critic, but this solution does not benefit from the additional information the state $s$ provides.
...
It is built on the foundation of another actor-critic method for MARL developed to solve a stochastic game called MADDPG \citep{lowe2017multi} for multi-agent deep deterministic policy gradient.
...
In short, since the action space is continuous, it is difficult to obtain $\argmax_u Q(s, u)$.
Still, it is possible to have a parametrised deterministic policy $\mu_\theta:\mathcal{S}\rightarrow\mathcal{U}$ updated by the gradient $\nabla_\theta Q^{\mu_\theta}(s, \mu_\theta(s))$ while the critic estimates this $Q(s, u)$ \citep{silver2014deterministic}.
...
The gradient to update each decentralised actor $\mu_a$ is different from the REINFORCE one given in Equation \ref{eq:ch3_policy_grad} and is
\begin{equation}
