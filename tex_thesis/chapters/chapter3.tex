...
\begin{figure}
    \centering
    \input{tex_thesis/figures/ch3/figure_envs}
...
Back to QPLEX,~\citet{wang2021qplex} demonstrated that if the advantage function $A$ respects the advantage-IGM, then the state action value function $Q = V + A$ respects IGM while removing constraints on the state value function $V$.
...
Other, such as QTRAN~\citep{Son2019QTRAN:Learning} and Weighted-QMIX~\citep{rashid2020weighted} factorise $Q_{tot}$ differently from the QMIX and VDN approach to improve the representational capacity of the factorisationn.
