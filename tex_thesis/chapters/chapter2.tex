...
Trying to summarise all definitions, we obtain that RL agents learn to cast by interacting with their environmental and sequentially taking actions that will modify the environment and provide them with a reward.
...
An important characteristic of MARL is that agents do not always observe the state $s_t$ of the environment entirely to select an action.
...
When multiple agents learn in the environment, many challenges arise and we identify four of them.
...
There is always a Nash equilibrium and in some environments, it is possible to have several.
When it is the case, one may provide a better sum of discounted rewards to all agents than another.
...
This manuscript covers these alternative solutions and their optimality in detail in Part \ref{part:compet}.
...
When the player $X$ chooses the action $B$, it receives a reward equal to 0 when player $Y$ chooses the action $A$, provided in the bottom left corner.
...
\begin{tabular}{cc|c|c|}
  ...
  \multirow{2}*{Player $X$} & $A$ & $(-1, -1)$ & $(-5, 0)$ \\\cline{3-4}
                            & $B$ & $(0, -5)$ & $(-2, -2)$ \\\cline{3-4}
\end{tabular}
...
A typical example of a normal-form cooperative game is ... with a payoff matrix provided in Table \ref{table:ch2_coopmatrix}.
...
An important property in MDP and SG not mentioned in the SG definition is that the next state depends only on the current state and action.
This is called the Markovian property.
It is important because it implies that a policy based solely on the current state is as good as one based on history of states and actions.
Moreover, we defined the policy as the probability of taking action in a given state.
A particular case of this stochastic policy is a deterministic policy, denoted $\pi(s):\mathcal{S}\rightarrow\mathcal{U}$, or standardly $\mu(s)$.
...
This reversible access means that the agent can execute an action in any state of the MDP and access the outcome at anytime.
...
Planning is the historical one, where methods build a local representation of their solution, typically by evaluating policies, resolving an optimisation problem, around a given state and discarding them after taking action.
...
The policy better than all others is the optimal policy.
...
It is, therefore, methods computing an approximate solution to the Belman optimality equations which are favored.

DP is an example of methods that compute the solutions by iteratively updating approximations, until it converges.
...
Alternating policy evaluation and improvement allows to iteratively compute the optimal policy, which is called policy iteration.
Policy iteration convergence time can be a big drawback in policy iteration.
...
Many more details about these DP methods are provided by \cite{sutton2018reinforcement}.
...
However, \cite{sutton2018reinforcement} claim that this take is somehow incorrect, explaining that DP has a polynomial complexity with respect to $|\mathcal{S}|$ and $|\mathcal{U}|$.
Moreover, computing them is impossible for continuous spaces without discretising the spaces.
Because of our direction towards model-free RL, without the model knowledge, the interest is to present these equations and methods that may ease the understanding of further concepts.
...
This is a characteristic of the off-policy methods, as opposed to the on-policy methods that improve the current policy based on samples from the current policy only.
In this manuscript, we consider only value-based methods that are off-policy, but on-policy methods are discussed further in Section~\ref{sec:ch2_policy_based_methods}.
...
Moreover, a baseline can be injected into the gradient to reduce variance without changing the expectation of the gradient.
...
To avoid approximating both $Q$ and $V$, it is possible to estimate the advantage with only one neural network is possible either by $A(s_t,u_t; \phi)=Q(s_t, u_t;\phi)-\sum_u \pi(u|s_t;\theta) Q(s_t,u; \phi)$ or by $A(s_t,u_t; \phi)=r_t +\gamma V(s_{t+1};\phi) - V(s_t;\phi)$.
This critic is often with value-based method, such as the ones defined in Section~\ref{sec:ch2_value_based_methods}.
...
Agents have complete access to the state $s$ of environment and perceive it without uncertainty.
...
As explained in Section \ref{sec:ch2_mdp}, policies are solely based on the current state in a MDP thanks to the Markovian property.
In a POMDP, the agent's policy $\pi$ can no longer be a function of the state.
Moreover, the observation does not necessarily hold the Markovian property and it could be suboptimal to act only based on the current observation.
...
Some authors implicitly include the current observation in $\tau$, allowing them to write $\pi(u|\tau)$ and $Q(\tau,u)$, to improve readability, we sometimes do it as well.
...
RNNs have many applications, and in POMDP, their hidden state allows maintaining a memory akin to a belief without processing the whole history at each time step.
...
This is because, with only the agent history, it is not possible to compute a belief of the state that is sufficient to take optimal actions.
...
\todo{Verifier qu'on a changer les $\rightarrow [0, 1]$ en $\Delta$}

\todo{Normal form game as a particular case of stateless SG and example of normal form games in the three settings?}

\todo{Remove we when talking about algorithms}

\todo{Figure after text (or before) but check consistency}

\todo{checl consistency table caption and style}

\todo{All figure avant dêtre ref ou apres.}