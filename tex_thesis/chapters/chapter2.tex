...
Before discussing challenges to learn in a SG, we hereafter provide some comments about its definition. 
...
Moreover, the expected return is often denoted $E_{\mathbf{\pi}} \left[ G_0 \right]$ leaving the the initial state distribution $p$ and the transition function $P$ depdendies implicit.
An important characteristic in MARL is that agents do not always observe the state $s_t$ of the environment entirely to select an action.
