...
An important property in MDP and SG not mentioned in the SG definition is that the next state is a dependent only of the current state and action.
This is called the Markovian property.
It is important because it implies that a policy based solely on the current state is as good as a policy based on the history of states and actions.
...
To provide some intuitions that will help understand later methods, we first present the development of value functions for any policy $\pi$ and $\forall s, u$, called the Bellman equation:
\begin{equation}
\label{eq:ch2_V_2}
\begin{split}
...
\end{split}
\end{equation}
Dynamic programming also rely on the property that the value functions $V^{\pi^*}$ and $Q^{\pi^*}$ of the optimal policy ${\pi^*}$ are the unique solution of the Bellman optimality equations $\forall s, u$:
\begin{equation}
